...
{
    public partial class tOrtu : UserControl
    {
        ...
        private void tOrtu_Load(object sender, EventArgs e)
        {
        ...
        private void btSelesai_Click_1(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.Yes)
            {
                if (pictureBox1.Image == null & pictureBox7.Image == null)
                {
                    ...
                    if (tbNama.Text != "" && tbOrtuDari.Text != "" && tbKeperluan.Text != "" && tbBertemu.Text != "" && tbHp.Text != "")
                    {
                        try
                        {
                            ...
                            command.CommandText = "INSERT INTO tb_ortu(nama_ortu, ortu_dari, keperluan, bertemu_dengan, no_hp, ttd, foto) VALUE (@nama, @ortudari, @keperluan, @bertemu, @no_hp, @ttd, @foto)";
                            conn.Open();

                            command.Parameters.Add("@nama", MySqlDbType.VarChar, 100);
                            command.Parameters.Add("@ortudari", MySqlDbType.VarChar, 100);
                            command.Parameters.Add("@keperluan", MySqlDbType.Text);
                            command.Parameters.Add("@bertemu", MySqlDbType.VarChar, 50);
                            ...
                            command.Parameters["@ortudari"].Value = tbOrtuDari.Text;
                            command.Parameters["@keperluan"].Value = tbKeperluan.Text;
                            command.Parameters["@bertemu"].Value = tbBertemu.Text;
                            command.Parameters["@no_hp"].Value = tbHp.Text;
                            command.Parameters["@ttd"].Value = ttd;
                            command.Parameters["@foto"].Value = foto;

                            MySqlDataReader reader = command.ExecuteReader();
                            MessageBox.Show("Berhasil Mengisi Form Tamu Orang Tua", "Informasi", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ...
                    else if (string.IsNullOrWhiteSpace(tbNama.Text) && string.IsNullOrWhiteSpace(tbOrtuDari.Text) && string.IsNullOrWhiteSpace(tbKeperluan.Text) && string.IsNullOrWhiteSpace(tbBertemu.Text) && string.IsNullOrWhiteSpace(tbHp.Text))
                    {
