...
{
    public partial class Undangan : UserControl
    {
        ...
        private void tbNoUndangan_OnTextChange(object sender, EventArgs e)
        {
        ...
        private void btSelesai_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.Yes)
            {
                if (pictureBox1.Image == null && pictureBox7.Image == null)
                {
                    ...
                    if (tbNoUndangan.Text != "" && tbNama.Text != "" && tbInstansi.Text != "" && tbHp.Text != "" && tbNoUndangan.Text != "" && tbPerihal.Text != "")
                    {
                        try
                        {
                            ...
                            command.CommandText = "INSERT INTO tb_undangan(no_undangan, nama, instansi, no_hp, waktu_diundang, ttd, perihal, foto) VALUE (@noundangan, @nama, @instansi, @no_hp, @waktudiundang, @ttd, @perihal, @foto)";
                            conn.Open();

                            command.Parameters.Add("@noundangan", MySqlDbType.VarChar, 15);
                            ...
                            command.Parameters.Add("@waktudiundang", MySqlDbType.Date);
                            command.Parameters.Add("@ttd", MySqlDbType.LongBlob);
                            command.Parameters.Add("@perihal", MySqlDbType.Text);
                            command.Parameters.Add("@foto", MySqlDbType.LongBlob);

                            command.Parameters["@noundangan"].Value = tbNoUndangan.Text;
                            ...
                            command.Parameters["@waktudiundang"].Value = dtWaktu.Value.ToString("yyyy-MM-dd");
                            command.Parameters["@ttd"].Value = ttd;
                            command.Parameters["@perihal"].Value = tbPerihal.Text;
                            command.Parameters["@foto"].Value = foto;

                            MySqlDataReader reader = command.ExecuteReader();
                            MessageBox.Show("Berhasil Mengisi Form Tamu Undangan", "informasi", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ...
                    else if (string.IsNullOrWhiteSpace(tbNoUndangan.Text) && string.IsNullOrWhiteSpace(tbNama.Text) && string.IsNullOrWhiteSpace(tbHp.Text) && string.IsNullOrWhiteSpace(tbInstansi.Text) && string.IsNullOrWhiteSpace(tbPerihal.Text))
                    {
